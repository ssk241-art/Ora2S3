...
def extract_snapshot_date_from_extra_where(extra_where: str) -> Optional[str]:
    """Extract snapshot date from extra_where clause if it matches
    SNAPSHOT_DT = DATE 'YYYY-MM-DD' pattern (case-insensitive).

    Returns the date as 'YYYYMMDD' string if found, None otherwise.

    Used by load.type='M' to route data/done files into the snapshot-date
    S3 partition instead of the current-date partition.
    """
    if not extra_where:
        return None
    match = re.search(r"(?i)\bSNAPSHOT_DT\s*=\s*DATE\s*'(\d{4}-\d{2}-\d{2})'", extra_where)
    if not match:
        return None
...
def table_to_s3(conn,
    ...
    # For load.type="M", determine S3 partition date:
    #   - If extra_where contains SNAPSHOT_DT = DATE 'YYYY-MM-DD', use that snapshot date
    #   - Otherwise, fall back to current date (America/New_York)
    run_partition_yyyymmdd = now_eastern_naive().strftime("%Y%m%d")
    if load_type == "M":
        extra_where_snapshot = extract_snapshot_date_from_extra_where(
            (get_config(source_cfg, "extra_where", "") or "").strip()
        )
        if extra_where_snapshot:
            run_partition_yyyymmdd = extra_where_snapshot
            log_message("[PARTITION] load.type=M snapshot_dt detected in extra_where "
                        "=> partition date={} (from extra_where)".format(run_partition_yyyymmdd))
        else:
            log_message("[PARTITION] load.type=M no snapshot_dt in extra_where "
                        "=> partition date={} (current date)".format(run_partition_yyyymmdd))
        ...
        try:
            if load_type == "M":
                ...
                run_tokens = {
                    ...
                    "yyyymmdd": run_partition_yyyymmdd,
                    "yyyymm": run_partition_yyyymmdd[:6],
                    "hhmm": "0000",
                }
    ...
    def _build_and_upload_aggregate_done_for_dest(dest: Dict[str, Any]):
        ...
        try:
            ...
            run_tokens["yyyymmdd"] = run_partition_yyyymmdd
            run_tokens["yyyymm"] = run_partition_yyyymmdd[:6]
            ...
            if bool(get_config(upload_cfg, "enabled", False)):
                ...
                done_upload_tokens["yyyymmdd"] = run_partition_yyyymmdd
                done_upload_tokens["yyyymm"] = run_partition_yyyymmdd[:6]
